...
def _invoke(n: int = _N_EXAMPLES, score: float = _DEFAULT_SCORE) -> Result[MonteCarloResult, ProcError]:
    llm = MagicMock(return_value=MagicMock())
    return MonteCarloEstimation().invoke(_make_dataset(n), llm, _make_scorer(score))
...
class TestInvoke:
    ...
    def test_returns_monte_carlo_result(self, result: MonteCarloResult) -> None:
    ...
    def test_scorer_call_count(self) -> None:
        scorer = _make_scorer()
        llm = MagicMock(return_value=MagicMock())
        MonteCarloEstimation().invoke(_make_dataset(), llm, scorer)
...
class TestClassification:
    ...
    def test_all_easy_when_perfect_score(self) -> None:
        assert _invoke().unwrap().easy == _N_EXAMPLES

    def test_all_hard_when_zero_score(self) -> None:
        assert _invoke(score=0).unwrap().hard == _N_EXAMPLES
