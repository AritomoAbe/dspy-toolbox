...
_STRESS_TEMPERATURE: float = 0.7


class BootstrapStressTestSuite(TestSuite):

    def __init__(self, dataset: TrainingSetDataset) -> None:
        config = BaseLLMConfig(
            name=MainModelNames.QWEN_3_4B_INSTRUCT,
            # stress test for prompt
            temperature=_STRESS_TEMPERATURE
        )
        ...
        nodes = [
            # Is this prompt robust or is it getting lucky?
...
def _main() -> None:
    ...
    BootstrapStressTestSuite(dataset).run()
