...
_RESPONSE_PREVIEW_LEN: int = 120
_DEFAULT_PORT: int = 8013
...
def chat_completions(request: ChatCompletionRequest) -> ChatCompletionResponse:
    ...
    dspy_response = _json.dumps({
        "reasoning": f"Generated by AbeGPT. Raw output: {generated[:_RESPONSE_PREVIEW_LEN].strip()}",
...
if __name__ == "__main__":
    ...
    LOGGER.info("Connect DSPy with:")
    LOGGER.info(f'  lm = dspy.LM("openai/gpt-abe", api_base="http://localhost:{args.port}", api_key="none")\n')
